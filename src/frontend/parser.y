...
%type <stmt> stmt exprStmt selStmt iterStmt jumpStmt forStmt
%type <stmtVec> stmts
...
funDef: type ID LPAREN params RPAREN LBRACE varDecs stmts RBRACE {
   ...
   * - Define the function by the ASTStatementBlock. */
   std::unique_ptr<ASTStatementBlock> statements = std::make_unique<ASTStatementBlock>(ASTStatementBlock());
   for(auto s : *$8) {
   ...
   auto f = ast.AddFunction($2, std::unique_ptr<VarType>($1), std::move(parameters), varidac);
  for(auto p : *$7) {
...
stmt: exprStmt {$$ = $1;} | LBRACE stmts RBRACE {
  ...
  $$ = statements;
 }| selStmt {$$ = $1;} | iterStmt {$$ = $1;} | jumpStmt {$$ = $1;} | forStmt {$$ = $1;} ;
...
selStmt: IF LPAREN expr RPAREN stmt {
  ...
  $$ = new ASTStatementIf(std::unique_ptr<ASTExpression>($3), std::unique_ptr<ASTStatement>($7), std::unique_ptr<ASTStatement>($5));
 };
...
forStmt: FOR LPAREN exprStmt orExpr SEMICOLON expr RPAREN stmt {
  // potentially push the last statement into the end of the body
