...
// TODO: Handle nullptrs and if only one is removed
bool ASTStatementIf::RemoveDead(std::vector<std::string>& live_list, bool lval)
{
    bool then = thenStatement && thenStatement->RemoveDead(live_list, false);
    bool elseS = elseStatement && elseStatement->RemoveDead(live_list, false);
    if(elseS && then)
    {
        return true;
    }
    ...
    if(then) 
    {
        thenStatement = std::unique_ptr<ASTStatement>(nullptr);
    }
    
    if(elseS) 
    {
        elseStatement = std::unique_ptr<ASTStatement>(nullptr);
    }

    condition.RemoveDead(live_list, false);
}