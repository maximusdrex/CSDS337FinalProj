...
bool ASTExpressionAssignment::RemoveDead(std::vector<std::string>& live_list, bool lval)
{
    // RemoveDead (lval=true) returns true when a variable on the left side was not found in the live list
    // If it returns false, it removes the variable from the live list
    if(left.RemoveDead(live_list, true)) {
        return true;
    } else {
        // When ran on the right side, RemoveDead adds all variables on the right side to the live list
        right.RemoveDead(live_list, false);
        return false;
    }
}