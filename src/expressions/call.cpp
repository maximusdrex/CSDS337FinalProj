...
bool ASTExpressionCall::RemoveDead(std::vector<std::string>& live_list, bool lval) 
{
    if(lval){
        throw std::runtime_error("ERROR: Function calls can't be lvals (DCE)");
    }
    // Add all paramaters to the live list and return false since function calls are assumed live
    // Side effects are possible so it has to be done this way

    for (auto it = arguments.begin(); it != arguments.end(); ++i) {
        // Add all variables passed as paramaters to the live_list
        // it needs to be dereferenced twice to access since it is a iterator pointing to a unique_ptr pointing to an ASTExpression
        (*it)->RemoveDead(live_list, false);
    }
