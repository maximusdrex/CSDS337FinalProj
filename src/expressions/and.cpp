...
llvm::Value* ASTExpressionAnd::Compile(llvm::IRBuilder<>& builder, ASTFunction& func) // Hm, this isn't the most efficient approach. I can think of a much easier way...
{
    ...
    builder.CreateCondBr(leftVal, checkRight, cont); // THIS IS THE ONLY CHANGE FROM OR: Instead of checking right on false, check right on true
...
bool ASTExpressionAnd::RemoveDead(std::vector<std::string>& live_list, bool lval)
{
    if(lval) {
        throw std::runtime_error("ERROR: Addition cannot be used as an lval");
    }
    a1.RemoveDead(live_list, false);
    a2.RemoveDead(live_list, false);
    return false;
}